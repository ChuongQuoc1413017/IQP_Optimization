...
def ising_obs(
    ...
    """
    Generate a list of Ising observables.

    Each observable is represented as a binary vector of length
    ``n_qubits``, where a value of ``1`` indicates a Z operator acting
    on that qubit and ``0`` indicates the identity.
    ...
    corr : int
        ...
        - ``1``: Include only single-qubit observables (Z_i).
        - ``2``: Include single-qubit observables and all pairwise
          observables (Z_i x Z_j).

    Returns
    -------
    np.ndarray
        Array of shape ``(n_observables, n_qubits)``, where each row
        encodes one observable.
    """
